...
public class SecuredMethodSpringBootIntegrationTest {
	...
	@WithUserDetails(value = "Manuel", userDetailsServiceBeanName = "userDetailsService")
	@Test
	public void givenAuthRequestWithUserDetail__shouldSucceedWith200() throws Exception {
		mvc.perform(get("/private/hello").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());

	}

	@Test
	public void givenAuthRequestWithUserRequestPostProcessor__shouldSucceedWith200() throws Exception {
		mvc.perform(get("/private/hello").with(user("Jose").password("12345").roles("USER"))
